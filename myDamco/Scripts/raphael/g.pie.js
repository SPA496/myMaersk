...
(function () {
    ...
    function Piechart(paper, cx, cy, r, values, opts) {
        ...
        var chartinst = this,
            ...
            defcut = true; // Whether we use the default "cut" = number of slices/legends is equal to the number of values - i.e. there are no "others" element. This is just a local variable used in a loop below.
        ...
        if (len == 1) {
            ...
            // adjust cut value, incase some values are too small percentage-wise, and we should put more items into "others" than given by the "maxSlices" threshold.
            for (i = 0; i < len; i++) {
                if (defcut && values[i] * 100 / total < minPercent) {
                    cut = Math.min(i, cut); // <- bugfix: putting values into "others" should never increase the number of slices we are going to show above the "maxSlices" threshold. So using min().
                    defcut = false;
                }
        ...
        var legend = function (labels, otherslabel, mark, dir) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                values[i].others && (labels[j] = otherslabel || "Others (" + (labels.length - i) + ")"); // KV: Added count
            ...
            chart.labels.translate.apply(chart.labels, [-50, 0]);

            var left = chart.labels.getBBox().x;
            if(left < 5) {
                chart.labels.translate.apply(chart.labels, [5 - left, 0]);
            };
